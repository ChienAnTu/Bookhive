...
const ComplaintDetailPage: React.FC = () => {
  ...
  const handleAdminAction = async () => {
    if (!complaint || !adminAction) return;

    try {
      let updatedComplaint;
      
      if (adminAction === "deduct") {
        const amount = parseFloat(deductAmount);
        if (isNaN(amount) || amount <= 0) {
          alert("Please enter a valid amount");
          return;
        }
        await deductDeposit(complaint.id, amount);
        // Reload complaint to see updated data
        updatedComplaint = await getComplaint(complaint.id);
        setComplaint(updatedComplaint);
        alert(`Deposit deduction of $${amount} has been processed.`);
      }
      
      // Update status if provided
      const statusMap: { [key: string]: "investigating" | "resolved" | "closed" } = {
        "investigating": "investigating",
        "resolved": "resolved",
        "closed": "closed"
      };
      
      const newStatus = statusMap[adminAction] || undefined;
      if (newStatus || adminResponse.trim()) {
        updatedComplaint = await updateComplaintStatus(
          complaint.id, 
          newStatus,
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Card className="shadow-sm mb-6">
          <div className="space-y-6">
            ...
            {complaint.deductedAmount && (
              <div>
                <h4 className="font-medium text-gray-900 mb-2">Deposit Deduction</h4>
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <div className="flex items-start space-x-3">
                    <DollarSign className="w-5 h-5 text-red-600 mt-0.5" />
                    <div>
                      <p className="text-red-900 font-medium">Amount Deducted: ${complaint.deductedAmount}</p>
                      {complaint.deductionReason && (
                        <p className="text-red-800 mt-1">{complaint.deductionReason}</p>
                      )}
                    </div>
                  </div>
                </div>
        ...
        {currentUser?.is_admin && showAdminPanel && (
          <Card className="shadow-sm bg-blue-50 border-blue-200">
            <h3 className="text-lg font-semibold mb-4 text-blue-900">Admin Actions</h3>
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  <option value="investigating">Mark as In Progress</option>
                  <option value="resolved">Mark as Resolved</option>
                  <option value="closed">Mark as Closed</option>
                  <option value="deduct">Deduct from Deposit</option>
                </select>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Provide response or explanation..."
                  rows={3}
                  className="w-full p-2 border border-blue-300 rounded-md focus:ring-2 focus:ring-blue-500"
                />
              </div>
